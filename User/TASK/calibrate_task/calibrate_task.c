...
static bool cali_head_hook(uint32_t *cali, bool cmd);   //表头校准数据函数
static bool cali_gyro_hook(uint32_t *cali, bool cmd);   //陀螺仪校准数据函数
static bool cali_gimbal_hook(uint32_t *cali, bool cmd); //云台校准数据函数

static const RC_ctrl_s *calibrate_RC; //遥控器结构体指针
...
//                             local_cali_t->pitch_max_angle, local_cali_t->pitch_min_angle);
    ...
    {
        if (1)/*(cmd_cali_gimbal_hook(&local_cali_t->yaw_offset, &local_cali_t->pitch_offset,
                                 &local_cali_t->yaw_max_angle, &local_cali_t->yaw_min_angle,
                                 &local_cali_t->pitch_max_angle, &local_cali_t->pitch_min_angle))*/
        {
            cali_buzzer_off();
