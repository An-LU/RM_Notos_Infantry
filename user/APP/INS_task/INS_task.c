...
static void IMU_temp_Control(fp32 temp);//抑制温漂，pwm升温加速度imu
...
//							  float pitch= *(INS_Angle+1);
//							  float ins = *(INS_gyro+1);
                //陀螺仪开机校准
                {
...
static void INS_Data_updata(void)
{
	...
	INS_Data.gyro_z = INS_gyro[INS_GYRO_Z_ADDRESS_OFFSET];
	
	INS_Data.accel_x = INS_accel[INS_ACCEL_X_ADDRESS_OFFSET];
	INS_Data.accel_y = INS_accel[INS_ACCEL_Y_ADDRESS_OFFSET];
	INS_Data.accel_z = INS_accel[INS_ACCEL_Z_ADDRESS_OFFSET];
}
const INS_Data_s *get_INS_Data_point(void)
{
	return &INS_Data;
}
